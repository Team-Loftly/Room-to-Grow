...
router.get("/", requireAuth, async (req, res) => {
    ...
    try {
        ...
        console.log(err);
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({message: "Error retrieving inventory"});
    }
});
...
router.post("/create", requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (existingInventory) {
            return res.status(StatusCodes.BAD_REQUEST).json({message: "Inventory already exists"});
        }
        ...
        const populatedInventory = await Inventory.findOne({ userId: user_id }).populate('decorations');
        res.status(StatusCodes.CREATED).json(populatedInventory);
    } catch (err) {
        console.log(err);
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({message: "Error creating inventor"});
    }
});
...
router.post("/update", requireAuth, async (req, res) => {
    ...
    try {
        ...
        console.log(err);
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({message: "Error updating inventory"});
    }
});
